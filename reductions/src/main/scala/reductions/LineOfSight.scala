...
object LineOfSight extends LineOfSightInterface:
  ...
  def lineOfSight(input: Array[Float], output: Array[Float]): Unit = {
      var max = 0f
      output(0) = 0
      for (index <- 1 until input.length) {
          def t = input(index) / index
          if (t < max) {
            output(index) = max
          } else {
            output(index) = t
  ...
  def upsweepSequential(input: Array[Float], from: Int, until: Int): Float = {
    ...
    for (index <- from until until) {
      max = Math.max(max, input(index) / index)
    }
    max
  }
  ...
  def upsweep(input: Array[Float], from: Int, end: Int,
    ...
    if (end - from <= threshold) Tree.Leaf(from, end, upsweepSequential(input, from, end))
      ...
      def mid = (end - from) / 2 + from
      var (left, right) = parallel(upsweep(input, from, mid, threshold), upsweep(input, mid, end, threshold))
  ...
  def parLineOfSight(input: Array[Float], output: Array[Float],
    ...
    def tree = upsweep(input, 1, input.length, threshold)
