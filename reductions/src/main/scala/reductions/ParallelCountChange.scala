...
object ParallelCountChange extends ParallelCountChangeInterface :
  ...
  def countChange(money: Int, coins: List[Int]): Int = {
    ...
    def countChange(remMoney: Int, remCoins: List[Int]): Int = {
      ...
      if (remMoney == 0) {
        1
      } else if (remCoins.isEmpty || remMoney < 0) {
        0
      } else
  ...
  def totalCoinsThreshold(totalCoins: Int): Threshold = (_, y) => y.length <= ((totalCoins * 2) / 3)


  /** Threshold heuristic based on the starting money and the initial list of coins. */
  def combinedThreshold(startingMoney: Int, allCoins: List[Int]): Threshold = (x, y) => x * y.length <= (startingMoney * allCoins.length / 2)
