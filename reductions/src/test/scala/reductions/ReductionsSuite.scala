...
import reductions.ParallelCountChange.Threshold
...
class ReductionsSuite extends munit.FunSuite:
  ...
  test("parCountChange") {
    def check(money: Int, coins: List[Int], theshold: Threshold) = {
      def c = parCountChange(money, coins, theshold)
      assert(parCountChange(money, coins, theshold) == 1,
        s"countChang($money, _) should be 1")
    }

    check(16, List(1), moneyThreshold(16))
  }
  ...
  test("parBalance should work for string of length 1") {
    ...
    check("f(hhjk(jk)i)", true)
    check("f(hhjk)jk)i)", false)
  }
