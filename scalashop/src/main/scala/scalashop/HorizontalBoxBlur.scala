...
object HorizontalBoxBlur extends HorizontalBoxBlurInterface:
  ...
  def parBlur(src: Img, dst: Img, numTasks: Int, radius: Int): Unit = {
    ...
    val ranges = for (index <- step to src.height by step)
      yield {
        val from = index - step
        val to = if (index + step > src.height) src.height else index
        (from, to)
      }

    ranges. map { p => task { blur(src, dst, p._1, p._2, radius) }}.foreach { t => t.join() }
  }
