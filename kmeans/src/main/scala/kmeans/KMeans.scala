...
class KMeans extends KMeansInterface:
  ...
  def classify(points: ParSeq[Point], means: ParSeq[Point]): ParMap[Point, ParSeq[Point]] = {
    ...
    def clustered = points.map(point => (findClosest(point, means), point))
      .groupBy( (mean, pointForMean) => mean).map(meanGroup => (meanGroup._1, meanGroup._2.map(pointPair => pointPair._2)))
    means.map(mean => (mean, clustered.getOrElse(mean, IndexedSeq().par))).toMap
  }
  ...
  final def kMeans(points: ParSeq[Point], means: ParSeq[Point], eta: Double): ParSeq[Point] = {
    ...
    def classified = classify(points, means)
    def updated = update(classified, means)
