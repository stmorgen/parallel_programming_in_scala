...
extends Quad:
  val mass = bodies.foldLeft(0f)( (s, b) => s + b.mass)
  val (massX, massY) = (bodies.foldLeft(0f)( (s, b) => s + b.x * b.mass) / mass : Float, bodies.foldLeft(0f)( (s, b) => s + b.y * b.mass) / mass : Float)
  ...
  def insert(b: Body): Quad = {
    if (size <= minimumSize)
      ...
      val newSize = size / 2f
      val centerOffset = size / 4f
      val newFork = Fork(
        Empty(centerX - centerOffset, centerY - centerOffset, newSize),
        Empty(centerX + centerOffset, centerY - centerOffset, newSize),
        Empty(centerX - centerOffset, centerY + centerOffset, newSize),
        Empty(centerX + centerOffset, centerY + centerOffset, newSize))
      bodies.appended(b).foldLeft(newFork)( (tmpFork, b) => tmpFork.insert(b))
    }
  }
...
class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) extends SectorMatrixInterface:
  ...
  def +=(b: Body): SectorMatrix =
    def intPos(p1: Float, p2: Float): Int = ((p1 - p2) / sectorSize).toInt
    this(intPos(b.x, boundaries.minX), intPos(b.y, boundaries.minY)) += b
    this
